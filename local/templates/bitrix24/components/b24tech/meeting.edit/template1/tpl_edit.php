...
?>
					...
					<table class="table-satatus-bar-in-edit-view">
						<tbody>
							<tr>
								<td>
									<label><?=GetMessage('ME_CU_STAGE')?></label>
								</td>
								<td>
												...
												<tbody>
													<tr>
														...
														<?foreach($stages as $key => $stage):?>
															<td class="crm-list-stage-bar-part" <?if($i <= $currentStage):?>style="background:#2FC6F6"<?endif;?>>
...
?>
				...
				<div class="meeting-new-members-block">
					<span class="meeting-new-members">
						<a class="meeting-new-members-left"><?=GetMessage('ME_CU_OWNER')?>:</a>
						<span class="meeting-new-members-right" id="meeting_owner"><?=$arResult['OWNER_FULL_NAME']?></span>
					</span>
					...
					<span class="meeting-new-members">
						<label for="COLLEGIATE"><?= getMessage("ME_CU_COLLEGIATE") ?></label>
                        <select id="COLLEGIATE" name="COLLEGIATE">
                            <option value="0"><?=getMessage("ME_CU_COLLEGIATE_0") ?></option>
                            <option value="1"><?=getMessage("ME_CU_COLLEGIATE_1") ?></option>
                        </select>
					</span>
				</div>
				<div class="meeting-new-meeting-plase">
                    <style>
                        input[name='PLACE'], #meeting_room_flag {
                            display: none;
                        }
                        input[name='INNER_PLACE']:checked ~ input[name='PLACE'] {
                            display: inline;
                        }
                    </style>
                    <span class="meeting-new-meeting-plase-text"><?=GetMessage('ME_PLACE')?></span>
                    <input style="width: auto; height: auto;" type="checkbox" id="INNER_PLACE" name="INNER_PLACE" <?=$arResult['MEETING']['PLACE']?'checked="checked"':''?>>
                    <label for="INNER_PLACE">Очное</label><br>
...
?>
					<div class="meeting-event-option">
						<input type="hidden" name="EVENT_REINVITE" value="N" /><input type="checkbox" checked="checked" name="EVENT_REINVITE" id="EVENT_REINVITE" value="Y" /><label for="EVENT_REINVITE"><?=GetMessage('ME_EVENT_REINVITE');?></label>
					</div>
<?
	endif;
?>
...
<?else:?>
	<br/>
	<input type="file" multiple="" name="FILES_MEETING[]">
	<br/>
<?endif;?>
...
?>
		...
		<div class="meeting-new-select-but-wrap">
			<a href="javascript:void(0)" onclick="createMeeting()" class="webform-button webform-button-create" id="meeting_save_button">
