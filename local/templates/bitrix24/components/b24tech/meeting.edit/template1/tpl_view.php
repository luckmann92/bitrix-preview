...
<script type="text/javascript" bxrunfirst="true">
    ...
    function UpdateAllUsersList(arUsers) {
        ...
        for (var i = 0; i < arUsers.length; i++) {
            ...
            if (s == 'K')
                inputs.appendChild(BX.create('INPUT', {
                    props: {
                        type: 'hidden',
                        name: 'KEEPERS[]',
                        value: arUsers[i].id
...
<div class="meetings-content">
    <div class="webform-round-corners webform-main-fields">
        ...
        <div class="webform-content meeting-detail-title-label"><?= GetMessage('ME_DESCR_TITLE') ?>
            <?
            if ($arResult['CAN_EDIT'] && $arResult['MEETING']['CURRENT_STATE'] == 'P'):
                ?>
                <a href="<?= $arParams['MEETING_EDIT_URL'] ?>"
                   class="meeting-edit-description"><?= GetMessage('ME_EDIT_TITLE') ?></a>
            <?
            endif;
            ?>
        </div>
    ...
    <div class="meetings-content">
        ...
        <div class="sidebar-block">
            ...
            <div class="sidebar-block-inner">
                <? /*
        <a href="/timeman/meeting/" class="webform-small-button webform-small-button-blue" style="margin-bottom: 15px;width: 100%;text-align: center;">
            <span class="webform-small-button-text"><?=getMessage("ME_LIST_TITLE")?></span>
        </a>
        */ ?>
                <?if ($arResult['ACCESS'] != 'M') {?>
                ...
                <div class="meeting-detail-info-users">
                    ...
                    <div class="meeting-detail-info-users-inner">
                        <div class="meeting-detail-info-users-title">
                            <span><?= GetMessage('ME_OWNER') ?></span><? /*<a class="webform-field-action-link" href=""><?=GetMessage('ME_CHANGE')?></a>*/ ?>
                        </div>
                ...
                <table cellspacing="0" class="meeting-detail-info-layout">
                    ...
                    <tr>
                        <td class="meeting-detail-left-column"><?= GetMessage('ME_CU_INTERNAL') ?>:</td>
                        <td class="meeting-detail-right-column"
                            id="meeting_state_text"><?= GetMessage('ME_CU_INTERNAL_' . ($arResult['MEETING']['PLACE'] == '' ? '0' : '1')) ?></td>
                        <!--                <td class="meeting-detail-right-column" id="meeting_state_text">-->
                        <? //=GetMessage('ME_CU_INTERNAL_' . ($arResult['MEETING']['PLACE'] ? '0' : '1')) ?><!--</td>-->
                    </tr>
                    <?
...
//if (strlen($arResult['MEETING']['PLACE']) > 0):
                    ...
                    ?>
                    <tr>
                        <td class="meeting-detail-left-column"><?= GetMessage('ME_CU_COLLEGIATE') ?>:</td>
                        <td class="meeting-detail-right-column"
                            id="meeting_state_text"><?= GetMessage('ME_CU_COLLEGIATE_' . $arResult['MEETING']['COLLEGIATE']) ?></td>
                    </tr>
                    <tr>
                        <td class="meeting-detail-left-column list-stage-bar-label"><?= GetMessage('ME_CURRENT_STATE') ?>
                            :
                        </td>
                        <td class="meeting-detail-right-column" id="meeting_state_text">
                        ...
                        <span class="main-grid-cell-content" data-prevent-default="false">
                                ...
                                <table class="crm-list-stage-bar-table">
                                    <tbody>
                                        <tr>
                                            ...
                                            $stages = array('P', 'A', 'C');
                                            $i = 0;
                                            $currentStage = array_search($arResult['MEETING']['CURRENT_STATE'], $stages);
                                            ?>
                                            <? foreach ($stages as $key => $stage): ?>
                                                <td class="crm-list-stage-bar-part"
                                                    <? if ($i <= $currentStage): ?>style="background:#2FC6F6"<? endif; ?>>
                            ...
                            <div class="crm-list-stage-bar-title"><?= GetMessage('MEETING_STATE_' . $arResult['MEETING']['CURRENT_STATE']) ?></div>
                        </span>
                    ...
                    if (strlen($arResult['MEETING']['GROUP_NAME']) > 0):
                        ...
                        <tr>
                            ...
                            <td class="meeting-detail-right-column"><a href="<?= $arResult['MEETING']['GROUP_URL'] ?>"
                                                                       class="meeting-detail-group-link"><?= $arResult['MEETING']['GROUP_NAME'] ?></a>
                            </td>
                        </tr>
                    <?
                    endif;
                    ...
                    <div class="meeting-detail-info-users-inner">
                        <div class="meeting-detail-info-users-title"><span><?= GetMessage('ME_KEEPER') ?></span>
                            ...
                            if ($arResult['CAN_EDIT']):
                                ?>
                                <a class="webform-field-action-link" href="javascript:void(0)"
                                   onclick="BXSelectKeepers(this)"><?= GetMessage('ME_CHANGE') ?></a>
                            <?
                            endif;
                            ?>
                        </div>
                        <div class="meeting-detail-info-users-list" id="meeting_users_K"></div>
                    </div>
                    <div class="meeting-detail-info-users-border"></div>
                </div>
                    ...
                    <div class="meeting-detail-info-users-inner">
                        <div class="meeting-detail-info-users-title"><span><?= GetMessage('ME_MEMBERS') ?></span>
                            ...
                            if ($arResult['CAN_EDIT']):
                                ?>
                                <a class="webform-field-action-link" href="javascript:void(0)"
                                   onclick="BXSelectMembers(this)"><?= GetMessage('ME_CHANGE') ?></a>
                            <?
                            endif;
                            ?>
                        </div>
                        <div class="meeting-detail-info-users-list" id="meeting_users_M"></div>
                    </div>
                    <div class="meeting-detail-info-users-border"></div>
                </div>
                <?
                if (count($arUsers['R']) > 0):
                    ...
                    <div class="meeting-detail-info-users meeting-refuse">
                        <div class="meeting-detail-info-users-border"></div>
                        <div class="meeting-detail-info-users-inner">
                            <div onclick="BX.toggle(BX('meeting_users_R')); BX.toggleClass(this,'meeting-refuse-close')"
                                 class="meeting-detail-info-users-title meeting-refuse-close">
                                <span class="meeting-refuse-title"><?= GetMessage('ME_REFUSED') ?> (<?= count($arUsers['R']) ?>)</span><span
                                        class="meeting-refuse-corner"></span>
                            </div>
    ...
    <?
    //dump($arResult['CURRENT_USER']['ROLE']);
    if ($arResult['CAN_EDIT']):
        ?>
        ...
        <div id="meeting_toolbar" class="meeting-toolbar toolbar-<?= $arResult['MEETING']['CURRENT_STATE'] ?>">
            ...
            <a href="javascript:void(0)"
               class="webform-small-button webform-small-button-accept meeting-agenda-bot-start"
               onclick="meetingAction('<?= $arParams['MEETING_ID'] ?>', {state: '<?= CMeeting::STATE_ACTION ?>'})">
                <span class="webform-small-button-left"></span>
                <span class="webform-small-button-text"><?= GetMessage('ME_ACTION') ?></span>
                <span class="webform-small-button-right"></span>
            </a>

            <a href="javascript:void(0)"
               class="webform-small-button webform-small-button-decline meeting-agenda-bot-stop"
               onclick="meetingAction('<?= $arParams['MEETING_ID'] ?>', {state: '<?= CMeeting::STATE_CLOSED ?>'})">
                <span class="webform-small-button-left"></span><span
                        class="webform-small-button-text"><?= GetMessage('ME_CLOSE') ?></span><span
                        class="webform-small-button-right"></span>
            </a>

            <div class="meeting-agenda-bot-return">
                <a href="<?= $arParams['MEETING_COPY_URL'] ?>"
                   class="meeting-agenda-bot-link meeting-dash-link"><?= GetMessage('ME_COPY') ?></a>
                &nbsp;
                <a onclick="meetingAction('<?= $arParams['MEETING_ID'] ?>', {state: '<?= CMeeting::STATE_PREPARE ?>'})"
                   href="javascript:void(0)"
                   class="meeting-agenda-bot-link meeting-dash-link"><?= GetMessage('ME_PREPARE') ?></a>
            </div>
        </div>
    <?
    endif;
