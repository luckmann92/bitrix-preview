...
if ($arResult['FROM_MEETING'] > 0):
    ?>
    <a href="<?
    echo str_replace('#MEETING_ID#', $arResult['FROM_MEETING'], $arParams['MEETING_URL_TPL']) ?>"
       class="webform-small-button webform-small-button-blue webform-small-button-back">
...
$canEdit = true;
if ($arResult['ITEM']['INSTANCES'][0]['MEETING']['CURRENT_STATE'] == 'C' //||
    /*$arResult['ITEM']['INSTANCES'][0]['MEETING']['ACCESS'] == 'M'*/) {
    $canEdit = false;
}

if ($arResult['ITEM']['INSTANCES'][0]['INSTANCE_PARENT_ID'] > 0) {

    $db_list = CIBlockSection::GetList(Array("ID" => "DESC"), array(
        "ACTIVE" => "Y",
        "IBLOCK_ID" => 30,
        "UF_ID_INSTANCE" => $arResult['ITEM']['INSTANCES'][0]['INSTANCE_PARENT_ID'],
    ...
    if ($ar_result = $db_list->Fetch()) {
        if ($ar_result['UF_STATUS'] != 'CLOSE') {
            $canEdit = false;
        }
    }
}
if (!$canEdit) {
   // echo '<span style="color:#ff0000">Нет доступа</span>';
   // return true;
}
?>
    ...
    <div class="meetings-content">
        <div class="webform-round-corners webform-main-fields">
            ...
            <div class="webform-content meeting-detail-title-label"><?= GetMessage('MI_BLOCK_TITLE') ?>
                <?
                if ($arResult['CAN_EDIT'] && $canEdit):
                    ?>
                    <span id="item_edit_link"
                          class="meeting-link meeting-edit-description"><?= GetMessage('MI_EDIT') ?></span>
                <?
                endif;
                ?>
            </div>
        ...
        <?
        if ($arResult['ITEM']['INSTANCES'][0]['MEETING']['KEEPER_ID'] != $USER->GetID()) {
            if ($arResult['ITEM']['INSTANCES'][0]['INSTANCE_PARENT_ID'] > 0) {
                $APPLICATION->IncludeComponent(
                    "b24tech:voting.form", "template1", array(
                        "INSTANCE_ID" => $arResult["ITEM"]["INSTANCES"][0]["ID"],
                        "MEETING_ID" => $arResult["ITEM"]["INSTANCES"][0]["MEETING"]["ID"],
                        "INSTANCE_PARENT_ID" => $arResult['ITEM']['INSTANCES'][0]['INSTANCE_PARENT_ID'],
                        "IS_QUESTION" => 'Y'
                    )
                );
            } else {
    ...
    <div class="item-edit-buttons" id="item_edit_buttons" style="display: none;">
        <a class="webform-small-button webform-small-button-accept" href="javascript:void(0)" onclick="viewItem()">
            <span class="webform-small-button-left"></span><span
                    class="webform-small-button-text"><?= GetMessage('MI_SAVE') ?></span><span
                    class="webform-small-button-right"></span>
        </a>
        <a href="javascript:void(0)" class="webform-small-button-link webform-button-link-cancel"
           onclick="resetItem(true); return false;"><?= GetMessage('MI_CANCEL') ?></a>
    </div>
<?
endif;
?>
...
<div class="meeting-ques-info-wrap">
    ...
    <table class="meeting-ques-info" cellspacing="0">
        ...
        <tr class="meeting-ques-info-top">
            ...
            <td class="meeting-ques-right"><a href="<?
                echo str_replace('#MEETING_ID#', $arInstance['MEETING_ID'], $arParams['MEETING_URL_TPL']) ?>"><?= htmlspecialcharsbx(GetMessage('MI_MEETING_TITLE', array('#ID#' => $arInstance['MEETING_ID'], '#TITLE#' => $arInstance['MEETING']['TITLE']))); ?></a>
            </td>
        </tr>
        <?
        //if ($arResult['ITEM']['CATEGORY']):
        ?>
        <tr class="meeting-ques-info-top">
            <td class="meeting-ques-left"><?= getMessage("MI_CU_REPORT_CATEGORY") ?>:</td>
            <td class="meeting-ques-right"><?= htmlspecialcharsbx($arResult['ITEM']['CATEGORY']); ?></td>
        </tr>
        <?
        //endif;
        ...
        <tr class="meeting-ques-info-top meeting-ques-info-top-last">
            ...
            <td class="meeting-ques-right">
                <div class="crm-list-stage-bar" id="CRM_LEAD_LIST_V12_PROGRESS_BAR_LEAD_4">
                    <table class="crm-list-stage-bar-table">
                        ...
                        <tr>
                            ...
                            $stages = array('P', 'A', 'C');
                            $i = 0;
                            $currentStage = array_search($arInstance['MEETING']['CURRENT_STATE'], $stages);
                            ?>
                            <?
                            foreach ($stages as $key => $stage):?>
                                <td class="crm-list-stage-bar-part" <?
                                if ($i <= $currentStage): ?>style="background:#2FC6F6"<?
                                endif;
                                ?>>
                                    <div class="crm-list-stage-bar-block">
                ...
                <div class="crm-list-stage-bar-title"><?= GetMessage("MEETING_STATE_" . $arInstance['MEETING']['CURRENT_STATE']) ?></div>
            </td>
            <!--					<td class="meeting-ques-right">-->
            <?//=GetMessage("MEETING_STATE_".$arInstance['MEETING']['CURRENT_STATE'])
            ?><!--</td>-->
        </tr>
        <tr class="meeting-ques-info-separator">
            <td colspan="2"></td>
        </tr>
        <tr class="meeting-ques-info-bottom meeting-ques-info-bottom-first">
            <td class="meeting-ques-left"><?= count($arInstance['RESPONSIBLE']) > 1 ? GetMessage('MI_REPORT_RESPONSIBLES') : GetMessage('MI_REPORT_RESPONSIBLE') ?>
                :
            </td>
            <td class="meeting-ques-right">
                <?
        ...
        //if (true):
            ...
            <tr class="meeting-ques-info-bottom-block meeting-ques-info-bot-last">
                <td colspan="2">
                    <form name="meeting_item_edit_<?= $arInstance['ID'] ?>" method="POST" enctype="multipart/form-data"
                ...
                <tr class="meeting-ques-info-bottom meeting-ques-info-bot-last">
                    ...
                    <td class="meeting-ques-right">
                        ...
                        foreach ($arReport['FILES'] as $ix => $arFile):
                            ?>
                            <div class="meeting-detail-file"><span
                                        class="meeting-detail-file-number"><?= $ix + 1 ?>.</span><span
                                        class="meeting-detail-file-info"><?
                                    if ($arFile['FILE_SRC']):?><a href="#message<?= $arFile['FILE_SRC'] ?>"
                                                                  class="meeting-detail-file-comment"></a><?endif ?><a
                                            class="meeting-detail-file-link"
                                            href="<?= $arFile['DOWNLOAD_URL'] ?>"><?= $arFile['ORIGINAL_NAME'] ?></a><span
                                            class="meeting-detail-file-size">(<?= $arFile['FILE_SIZE_FORMATTED'] ?>)</span></span>
                            </div>
                        <?
                        endforeach;
                        ?>
    ...
    if ($arInstance['B_EDIT']):
        $bEdit = true;
        ?>
        <div class="meeting-question-buttons"><a class="webform-small-button webform-small-button-accept"
                                                 href="javascript:void(0)"
                                                 onclick="saveReport(this, document.forms.meeting_item_edit_<?= $arInstance['ID'] ?>)"><span
                        class="webform-small-button-left"></span><span
                        class="webform-small-button-text"><?= GetMessage('MI_SAVE') ?></span><span
                        class="webform-small-button-right"></span></a></div>
    <?
    endif;
    ?>
</div>
...
<div class="meeting-question-history-wrap">
    ...
    ?>
    <span class="meeting-link meeting-quest-hist-hide"
          onclick="BX.toggle(BX('meeting_question_history')); this.innerHTML=BX.toggle(this.innerHTML, <?= CUtil::PhpToJsObject($t) ?>);"><?= $t[0] ?></span>
        ...
        function saveReport(el, form) {
            if (BX.hasClass(el, 'webform-small-button-accept')) {
                ...
                BX.ajax.submit(form, function (d) {
                    ...
                    el.parentNode.appendChild(BX.create('SPAN', {
                        props: {className: 'meeting-report-saved'},
                        text: '<?=CUtil::JSEscape(GetMessage('MI_SAVED'))?>'.replace('#TIME#', BX.date.format(BX.isAmPmMode() ? 'h:i a' : 'H:i'))
                    }));
                });
            }
        }
