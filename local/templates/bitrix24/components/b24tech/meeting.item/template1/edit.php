...
<div class="meeting-detail-category-wrap">
	<span class="meeting-ques-inp-label"><?=GetMessage('MI_EDIT_CATEGORY')?></span>
    <select name="ITEM_CATEGORY" class="meeting-ques-edit-inp" maxlength="255">
<?
foreach ($arResult['CATEGORIES'] as $arCategory):
?>
        <option value=<?=$arCategory['ID']?> <?=$arResult['ITEM']['CATEGORY_ID'] == $arCategory['ID'] ? 'selected="selected"' : '' ?>><?=$arCategory['TITLE']?></option>
<?
endforeach;
?>
    </select>
