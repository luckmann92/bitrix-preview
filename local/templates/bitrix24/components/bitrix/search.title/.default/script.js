...
BX.B24SearchTitle = function(arParams)
{
	...
	this.BuildEntities = function (result)
	{
		...
		var crmContactMore = false, crmCompanyMore = false, crmDealMore = false, crmLeadMore = false,
			crmInvoiceMore = false, crmQuoteMore = false, diskMore = false;
		...
		if (result.data.length > 0)
		{
			for (var i in result.data)
			{
				...
				var item = {
					"NAME": BX.util.htmlspecialchars(result.data[i].title),
				...
				else if (result.data[i].entityType == "DISK")
				{
			...
			this.BuildEntityBlock(diskItems, "DISK", "disk");
			if (diskMore)
			{
				item = {
