	...
	BX.Tasks.Component.TasksWidgetButtonsTask = BX.Tasks.Util.Widget.extend({
		...
		methods: {
			...
			doDynamicAction: function(code, args)
			{
				...
				this.hideAgendaError();
				if (code == 'APPROVE_AGENDA' || code == 'REJECT_AGENDA') {
					if (code == 'REJECT_AGENDA') {
						var comments = document.querySelectorAll('[id^=record-TASK].feed-com-block-cover');
						if (comments.length <= 0) {
							this.showAgendaError();
							return false;
						}
					}
					if (parseInt(this.opts.meeting_id) > 0) {
						BX.ajax({
							url: this.opts.templateUrl + '/notice-ajax.php',
							data: {
								ACTION: 'SEND_NOTICE',
								MEETING_ID: parseInt(this.opts.meeting_id),
								CODE: code,
							},
							method: 'POST',
							dataType: 'json',
					...
					var buttons = document.querySelectorAll('.task-detail-buttons span[data-bx-id="task-view-b-button"]');
					buttons.forEach(
						function (button) {
			...
			hideAgendaError: function()
			{
				var commentsBlock, errorBlock;
				commentsBlock = document.querySelector('.feed-com-add-box');
				BX.removeClass(commentsBlock, 'error');
				errorBlock = document.querySelector('.error-message-block');
				BX.remove(errorBlock);
			},

			showAgendaError: function()
			{
				var commentsBlock, errorBlock;
				commentsBlock = document.querySelector('.feed-com-add-box');
				BX.addClass(commentsBlock, 'error');
				errorBlock = BX.create('div', {
					props: {
						className: 'error-message-block',
					},
					style: {},
					text: BX.message('AGENDA_EMPTY_COMMENT_BLOCK'),
				});
				BX.append(errorBlock, commentsBlock);
			},
