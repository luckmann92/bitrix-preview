...
$arCategories = CMeetingItem::GetCategories();
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && $_REQUEST['save_item'] && check_bitrix_sessid())
{
	...
	if ($arResult['CAN_EDIT'])
	{
		...
		if (isset($_REQUEST['ITEM_CATEGORY']))
			$arResult['ITEM']['CATEGORY_ID'] = $arFields['CATEGORY_ID'] = intval($_REQUEST['ITEM_CATEGORY']);

        foreach ($arCategories as $k => $arCategory) {
            if ($arCategory['ID'] == $arResult['ITEM']['CATEGORY_ID']) {
                $arResult['ITEM']['CATEGORY'] = $arCategory['TITLE'];
                break;
            }
        }
	}
