...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_REQUEST['save']) && $arResult['CAN_EDIT'] && check_bitrix_sessid())
{
	...
	if ($bFromEditForm)
	{
		...
		$arFields = array(
			...
			'PLACE' => $_REQUEST['INNER_PLACE'] == 'on' ? $_REQUEST['PLACE'] : "",
			'GROUP_ID' => $_REQUEST['GROUP_ID'],
            'COLLEGIATE' => $_REQUEST['COLLEGIATE'],
		);

		if(strlen($_REQUEST['PLACE_ID']) > 0 && $_REQUEST['INNER_PLACE'] == 'on')
		...
		if ($arParams['COPY'])
		{
            ...
            $arOwner = CUser::GetByID($arResult['MEETING']['OWNER_ID'])->GetNext();
		...
		{
		    //Создание чата при создании заседания
...
//                $avatarId = \CFile::SaveFile(\CFile::MakeFileArray($pic), 'im');
                ...
                $chatId = $chat->Add(array(
                    'TITLE' => 'Заседание: '.$arFields['TITLE'],
                    'DESCRIPTION' => 'Чат заседания '.$arFields['TITLE'],
                    'COLOR' => 'MINT',//цвет
                    'TYPE' => IM_MESSAGE_CHAT,//тип чата
                    'AUTHOR_ID' => $arOwner['ID'],//владелец чата
//                    'AVATAR_ID' => $avatarId,//аватарка чата
                ...
                \CIMChat::AddSystemMessage(array(
                    ...
                    'MESSAGE' => 'Чат заседания "'.$arFields['TITLE'].'" успешно создан!',
                ));
            ...
            else echo "Модуль IM не подключился :(";
	...
	if ($res)
	{
		...
		if (count($USERS) > 0)
		{
			...
			foreach(GetModuleEvents("meeting", "OnAfterMeetingAddComponent", true) as $arEvent)
            {
                ExecuteModuleEventEx($arEvent, array($MEETING_ID, $files));
            }
		}
