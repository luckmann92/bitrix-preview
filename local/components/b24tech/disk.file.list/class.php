...
class CDiskFileListComponent extends BaseComponent
{
	...
	protected function processActionDefault()
	{
		...
		if($this->arResult['ENABLED_MOD_ZIP'] && !$this->editMode)
		{
			...
			$this->arResult['DOWNLOAD_ARCHIVE_URL'] = '/local/components/b24tech/disk.file.list/archive.php?folder='.$this->arParams['FOLDER_ID'];
	...
	{
		if(empty($this->arParams['FOLDER_ID']) && !($this->arParams['FOLDER_ID'] > 0))
		{
			return array();
		}
		...
		$folder = \Bitrix\Disk\Folder::loadById($this->arParams['FOLDER_ID']);
		if(!$folder)
		{
			return array();
		}
		...
		))) as $fileModel)
		{
			...
			$data = array(
				...
				'VIEW_URL' => $urlManager->getUrlToShowFileByService($id, 'gvdrive'),
				'EDIT_URL' => $urlManager->getUrlForStartEditFile($id, 'gdrive'),
				'DOWNLOAD_URL' => $urlManager->getUrlForDownloadFile($fileModel),
				//'COPY_TO_ME_URL' => $urlManager->getUrlUfController('copyToMe', array('attachedId' => $id)),

				'DELETE_URL' => $urlManager->getUrlForStartEditFile($id, 'gdrive')
			);

			if(!$this->arParams['NO_IMAGES'] && \Bitrix\Disk\TypeFile::isImage($fileModel))
			{
			...
			elseif(!$this->editMode && $fileModel)
			{
				...
				$data['CURRENT_USER_IS_OWNER'] = true;
				$data['ALLOW_AUTO_COMMENT'] = true;
				...
				$data['ATTRIBUTES_FOR_VIEWER'] = Ui\Viewer::getAttributesByObject($fileModel, $additionalParams);
			}
