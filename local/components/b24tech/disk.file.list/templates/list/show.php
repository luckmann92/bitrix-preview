...
<div id="disk-attach-block-<?=$arResult['UID']?>" class="feed-com-files diskuf-files-entity"><?
	...
	if (!empty($arResult['FILES']) || !empty($arResult['DELETED_FILES']))
	{
		...
		foreach($arResult['FILES'] as $file)
		{
			...
			?><div class="<?=$className?>" id="js-file-<?=$file['ID']?>" style="display: block;">
				...
				<span class="feed-com-file-name-wrap">
					...
					<a href="javascript:void(0);" class="feed-com-file-size" onclick="deleteFileById(<?=$file['ID']?>)">Удалить</a>
...
<script type="text/javascript">
	function deleteFileById(id) {
		BX.showWait();
		BX.ajax.get(
		 '/local/components/b24tech/disk.file.list/delete_file.php',
		 { file: id },
		 function (data) {
		 	BX.closeWait();
		 	var res = JSON.parse(data);
		 	if(res.success) {
		 		BX('js-file-'+id).remove();
		 	}
		 }
		);
	}
