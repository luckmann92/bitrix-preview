...
$dbUsers = CIntranetUtils::GetSubordinateEmployees('', true, 'Y', array('ID'));
...
if ($_REQUEST['FILTER'])
{
	...
	if (isset($arFilterValues['KEEPER_ID']) && intval($arFilterValues['KEEPER_ID']) > 0)
		$arResult['FILTER']['KEEPER_ID'] = $arFilter['KEEPER_ID'] = intval($arFilterValues['KEEPER_ID']);
    ...
    if (isset($arFilterValues['DATE_START']['FROM']) && $arFilterValues['DATE_START']['FROM'] != '')
    {
        $arResult['FILTER']['DATE_START']['FROM'] = $arFilter['>DATE_START'] = $arFilterValues['DATE_START']['FROM'];
    }
    if (isset($arFilterValues['DATE_START']['TO']) && $arFilterValues['DATE_START']['TO'] != '')
    {
        $arResult['FILTER']['DATE_START']['TO'] = $arFilter['<DATE_START'] = $arFilterValues['DATE_START']['TO'];
    }


    if (isset($arFilterValues['INTERNAL']) && $arFilterValues['INTERNAL'] != "-1")
    {
        $arResult['FILTER']['INTERNAL'] = $arFilterValues['INTERNAL'];
        if (!intval($arFilterValues['INTERNAL']))
            $arFilter['PLACE'] = false;
        else
            $arFilter['!PLACE'] = false;
    }
    if (isset($arFilterValues['COLLEGIATE']) && $arFilterValues['COLLEGIATE'] != "-1")
    {
        $arResult['FILTER']['COLLEGIATE'] = $arFilter['COLLEGIATE'] = intval($arFilterValues['COLLEGIATE']);
    }
}
...
$dbRes = CMeeting::GetList(
	...
	false,
	$arNavParams,
	array('ID', 'TITLE', 'CURRENT_STATE', 'DATE_START', 'OWNER_ID', 'PLACE', 'COLLEGIATE')
);
