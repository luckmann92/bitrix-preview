...
$answers = array(
	array('TITLE'=> 'Да', 'VALUE' => 'Y'),
	array('TITLE'=> 'Нет', 'VALUE' => 'N'),
	array('TITLE'=> 'Воздерживаюсь', 'VALUE' => 'A'),
);
...
<?if(!empty($arResult['QUESTIONS'])):?>
	...
	<div id="js-voting" class="form-questions">
		<select class="member-select" name="member" id="js-member">
			<?foreach($arResult['MEMBERS'] as $id => $name):?>
				...
				<?foreach($answers as $answer):?>
					<div class="form-check">
					  <input 
					  	...
					  	>
					  <label class="form-check-label" for="id-question-<?=$question['ID']?>">
					  	<?=$answer['TITLE']?>
					  </label>
					</div>
				<?endforeach;?>
	...
	<script type="text/javascript">
		$(document).ready(function() {
			var bulletin = new VoitingBulletin(<?=$arResult["USER_ID"];?>, <?=$arResult["MEETING_ID"];?>, <?=$arResult["SECTION_ID"];?>, 'offline');
		});
	</script>
<?endif;?>