...
class VotingComponent extends CBitrixComponent
{
	...
	public function executeComponent()
	{
		if($this->arParams["MEETING_ID"] > 0 && $this->arParams["INSTANCE_ID"] > 0) {
			...
			/*if(($userRole == 'K' || $userRole == 'O') && $voting['UF_STATUS'] == 'FINISHED') {
			...
			if($voting['UF_STATUS'] == 'IN_PROCESS' && ($userRole == 'M')) {
			...
			if( ($voting['UF_STATUS'] == 'IN_PROCESS' || $voting['UF_STATUS'] == 'FINISHED') && ($userRole == 'K' || $userRole == 'O') && $voting['UF_TYPE'] == 'offline') {
				...
				if(count($questions) > 0) {
					if($this->arParams["IS_QUESTION"] == "Y")
                        $questions = array_filter($questions, function($item){ return $item['PROPERTY_ID_INSTANCE_VALUE'] == $this->arParams['INSTANCE_ID']; });
                    $this->arResult["QUESTIONS"] = $questions;
                    
					$ids = $this->getMembers($this->arParams["MEETING_ID"]);
					foreach ($ids as $id) {
        				$ids_questions = $this->detectUnansweredQuestions($id, $voting["ID"], $questions);
					    if (count($ids_questions) > 0) {
                            $filteredIds[] = $id;
					    }
                    }
					$ids = $filteredIds;
		...
		if($this->arParams['ACTION'] === 'create_all' && !empty($this->arParams['MEETING_ID']))
		{
			$this->mode = $this->arParams['TYPE_VOTING'];

			$parentTnstances = $this->getAllParentInstances($this->arParams['MEETING_ID']);
			
			foreach($parentTnstances as $parentInstance) {
				$instances = $this->getInstances($parentInstance);
				if(!empty($instances['CHILD'])) {
					$this->createNewVoiting($instances['PARENT'], $instances['CHILD']);
				}
	...
	/*protected function getAllParentInstances($meetingId)
	{
		$instances = array();
		if($meetingId > 0) {
			$res = CMeeting::GetItems($meetingId);
			while ($item = $res->Fetch()) {
				if(!$item["INSTANCE_PARENT_ID"]) {
					$instances[] = $item;
				}
			}
		}
	...
	protected function getAllParentInstances($meetingId) 
	{
		$instances = array();
		if($meetingId > 0) {
			$res = CMeetingInstance::GetList(array(), array("MEETING_ID" => $meetingId, 'INSTANCE_PARENT_ID'=>0));
			while ($item = $res->Fetch()) {
				$instances[] = $item;
			}
		}
