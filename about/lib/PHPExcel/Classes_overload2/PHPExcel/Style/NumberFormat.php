...
class PHPExcel_Style_NumberFormat extends PHPExcel_Style_Supervisor implements PHPExcel_IComparable
{
    ...
    private static function formatAsPercentage(&$value, &$format)
    {
        if ($format === self::FORMAT_PERCENTAGE) {
            ...
            if (preg_match('/\.[#0]+/i', $format, $m)) {
                $s = mb_orig_substr($m[0], 0, 1) . (mb_orig_strlen($m[0]) - 1);
                $format = str_replace($m[0], $s, $format);
            }
            if (preg_match('/^[#0]+/', $format, $m)) {
                $format = str_replace($m[0], mb_orig_strlen($m[0]), $format);
            }
    ...
    private static function formatAsFraction(&$value, &$format)
    {
        ...
        $decimalLength = mb_orig_strlen($decimalPart);
        ...
        if ((mb_orig_strpos($format, '0') !== false) || (mb_orig_strpos($format, '#') !== false) || (mb_orig_substr($format, 0, 3) == '? ?')) {
            if ($integerPart == 0) {
                $integerPart = '';
            }
        ...
        if (preg_match('/(\[\$[A-Z]*-[0-9A-F]*\])*[hmsdy](?=(?:[^"]|"[^"]*")*$)/miu', $format, $matches)) {
            ...
            if ($format === self::FORMAT_CURRENCY_EUR_SIMPLE) {
                ...
                if (preg_match('/(#|0)(,+)/', $format, $matches)) {
                    $scale = pow(1000, mb_orig_strlen($matches[2]));
                ...
                if (preg_match('/#?.*\?\/\?/', $format, $m)) {
                    ...
                    if (preg_match($number_regex, $m, $matches)) {
                        ...
                        $minWidth = mb_orig_strlen($left) + mb_orig_strlen($dec) + mb_orig_strlen($right);
                        if ($useThousands) {
                            $value = number_format(
                                $value,
                                mb_orig_strlen($right),
                            ...
                            if (preg_match('/[0#]E[+-]0/i', $format)) {
                                ...
                                $sprintf_pattern = "%0$minWidth." . mb_orig_strlen($right) . "f";
