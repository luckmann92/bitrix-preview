...
class PHPExcel_Calculation_Engineering
{
    ...
    public static function parseComplex($complexNumber)
    {
        ...
        $suffix = mb_orig_substr($workString, -1);
        if (!is_numeric($suffix)) {
            $workString = mb_orig_substr($workString, 0, -1);
        } else {
            $suffix = '';
        }
        ...
        $leadingSign = 0;
        if (mb_orig_strlen($workString) > 0) {
            $leadingSign = (($workString{0} == '+') || ($workString{0} == '-')) ? 1 : 0;
        }
        $power = '';
        $realNumber = strtok($workString, '+-');
        if (mb_orig_strtoupper(mb_orig_substr($realNumber, -1)) == 'E') {
            $power = strtok('+-');
            ++$leadingSign;
        }

        $realNumber = mb_orig_substr($workString, 0, mb_orig_strlen($realNumber)+mb_orig_strlen($power)+$leadingSign);

        if ($suffix != '') {
            $imaginary = mb_orig_substr($workString, mb_orig_strlen($realNumber));
    ...
    {
        if ($complexNumber{0} == '+') {
            $complexNumber = mb_orig_substr($complexNumber, 1);
        }
    ...
    {
        if (!is_null($places)) {
            if (mb_orig_strlen($xVal) <= $places) {
                return mb_orig_substr(str_pad($xVal, $places, '0', STR_PAD_LEFT), -10);
            } else {
                return PHPExcel_Calculation_Functions::NaN();
            }
        }

        return mb_orig_substr($xVal, -10);
    }
    ...
    public static function BINTODEC($x)
    {
        ...
        $x = (string) $x;
        if (mb_orig_strlen($x) > preg_match_all('/[01]/', $x, $out)) {
            return PHPExcel_Calculation_Functions::NaN();
        }
        if (mb_orig_strlen($x) > 10) {
            return PHPExcel_Calculation_Functions::NaN();
        } elseif (mb_orig_strlen($x) == 10) {
            //    Two's Complement
            $x = mb_orig_substr($x, -9);
            return '-'.(512-bindec($x));
        }
        return bindec($x);
    }
    ...
    public static function BINTOHEX($x, $places = null)
    {
        ...
        if (mb_orig_strlen($x) > 10) {
            ...
            //    Two's Complement
            return str_repeat('F', 8).mb_orig_substr(mb_orig_strtoupper(dechex(bindec(mb_orig_substr($x, -9)))), -2);
        }
        $hexVal = (string) mb_orig_strtoupper(dechex(bindec($x)));
    ...
    public static function BINTOOCT($x, $places = null)
    {
        ...
        if (mb_orig_strlen($x) > 10) {
            ...
            //    Two's Complement
            return str_repeat('7', 7).mb_orig_substr(mb_orig_strtoupper(decoct(bindec(mb_orig_substr($x, -9)))), -3);
        }
    ...
    public static function DECTOBIN($x, $places = null)
    {
        ...
        $x = (string) $x;
        if (mb_orig_strlen($x) > preg_match_all('/[-0123456789.]/', $x, $out)) {
            return PHPExcel_Calculation_Functions::VALUE();
        }
        $x = (string) floor($x);
        $r = decbin($x);
        if (mb_orig_strlen($r) == 32) {
            //    Two's Complement
            $r = mb_orig_substr($r, -10);
        } elseif (mb_orig_strlen($r) > 11) {
            return PHPExcel_Calculation_Functions::NaN();
        }
    ...
    public static function DECTOHEX($x, $places = null)
    {
        ...
        $x = (string) floor($x);
        $r = mb_orig_strtoupper(dechex($x));
        if (mb_orig_strlen($r) == 8) {
            //    Two's Complement
            $r = 'FF'.$r;
        }
    ...
    public static function HEXTOBIN($x, $places = null)
    {
        ...
        $x = (string) $x;
        if (mb_orig_strlen($x) > preg_match_all('/[0123456789ABCDEF]/', mb_orig_strtoupper($x), $out)) {
            return PHPExcel_Calculation_Functions::NaN();
        }
        $binVal = decbin(hexdec($x));

        return mb_orig_substr(self::nbrConversionFormat($binVal, $places), -10);
    }
    ...
    public static function OCTTOBIN($x, $places = null)
    {
        ...
        $x = (string) $x;
        if (preg_match_all('/[01234567]/', $x, $out) != mb_orig_strlen($x)) {
            return PHPExcel_Calculation_Functions::NaN();
        }
        $r = decbin(octdec($x));
    ...
    public static function OCTTOHEX($x, $places = null)
    {
        ...
        $hexVal = mb_orig_strtoupper(dechex(octdec($x)));
    ...
    public static function CONVERTUOM($value, $fromUOM, $toUOM)
    {
        ...
        if (isset(self::$conversionUnits[$fromUOM])) {
            ...
            $fromMultiplier = mb_orig_substr($fromUOM, 0, 1);
            $fromUOM = mb_orig_substr($fromUOM, 1);
        ...
        if (isset(self::$conversionUnits[$toUOM])) {
            ...
            $toMultiplier = mb_orig_substr($toUOM, 0, 1);
            $toUOM = mb_orig_substr($toUOM, 1);
