...
class PHPExcel_Calculation_LookupRef
{
    ...
    public static function CELL_ADDRESS($row, $column, $relativity = 1, $referenceStyle = true, $sheetText = '')
    {
        ...
        if ($sheetText > '') {
            if (mb_orig_strpos($sheetText, ' ') !== false) {
                $sheetText = "'".$sheetText."'";
            }
            $sheetText .='!';
        }
    ...
    public static function COLUMN($cellAddress = null)
    {
        ...
        if (is_array($cellAddress)) {
            ...
            if (mb_orig_strpos($cellAddress, '!') !== false) {
    ...
    public static function MATCH($lookup_value, $lookup_array, $match_type = 1)
    {
        ...
        $lookup_value = mb_orig_strtolower($lookup_value);
        ...
        foreach ($lookup_array as $i => $lookupArrayValue) {
            ...
            if (is_string($lookupArrayValue)) {
                $lookup_array[$i] = mb_orig_strtolower($lookupArrayValue);
            }
    ...
    private static function vlookupSort($a, $b)
    {
        reset($a);
        $firstColumn = key($a);
        if (($aLower = mb_orig_strtolower($a[$firstColumn])) == ($bLower = mb_orig_strtolower($b[$firstColumn]))) {
            return 0;
        }
    ...
    public static function VLOOKUP($lookup_value, $lookup_array, $index_number, $not_exact_match = true)
    {
        ...
        foreach ($lookup_array as $rowKey => $rowData) {
            if ((is_numeric($lookup_value) && is_numeric($rowData[$firstColumn]) && ($rowData[$firstColumn] > $lookup_value)) ||
                (!is_numeric($lookup_value) && !is_numeric($rowData[$firstColumn]) && (mb_orig_strtolower($rowData[$firstColumn]) > mb_orig_strtolower($lookup_value)))) {
                break;
            }
            $rowNumber = $rowKey;
    ...
    public static function HLOOKUP($lookup_value, $lookup_array, $index_number, $not_exact_match = true)
    {
        ...
        foreach ($lookup_array[$firstColumn] as $rowKey => $rowData) {
            if ((is_numeric($lookup_value) && is_numeric($rowData) && ($rowData > $lookup_value)) ||
                (!is_numeric($lookup_value) && !is_numeric($rowData) && (mb_orig_strtolower($rowData) > mb_orig_strtolower($lookup_value)))) {
                break;
            }
            $rowNumber = $rowKey;
            $rowValue = $rowData;
