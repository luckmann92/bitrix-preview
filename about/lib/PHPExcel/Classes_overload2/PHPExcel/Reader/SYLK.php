...
class PHPExcel_Reader_SYLK extends PHPExcel_Reader_Abstract implements PHPExcel_Reader_IReader
{
    ...
    protected function isValidFormat()
    {
        ...
        $delimiterCount = mb_orig_substr_count($data, ';');
        if ($delimiterCount < 1) {
            return false;
        }
        ...
        $lines = explode("\n", $data);
        if (mb_orig_substr($lines[0], 0, 4) != 'ID;P') {
            return false;
        }
            ...
            if ($dataType == 'C') {
                ...
                foreach ($rowData as $rowDatum) {
                    switch ($rowDatum{0}) {
                        case 'C':
                        case 'X':
                            $columnIndex = mb_orig_substr($rowDatum, 1) - 1;
                            break;
                        case 'R':
                        case 'Y':
                            $rowIndex = mb_orig_substr($rowDatum, 1);
                            break;
                    }
        ...
        while (($rowData = fgets($fileHandle)) !== false) {
            ...
            if ($dataType == 'P') {
                ...
                foreach ($rowData as $rowDatum) {
                    switch ($rowDatum{0}) {
                        case 'P':
                            $formatArray['numberformat']['code'] = str_replace($fromFormats, $toFormats, mb_orig_substr($rowDatum, 1));
                            break;
                        case 'E':
                        case 'F':
                            $formatArray['font']['name'] = mb_orig_substr($rowDatum, 1);
                            break;
                        case 'L':
                            $formatArray['font']['size'] = mb_orig_substr($rowDatum, 1);
                            break;
                        case 'S':
                            $styleSettings = mb_orig_substr($rowDatum, 1);
                            for ($i=0; $i<mb_orig_strlen($styleSettings); ++$i) {
            ...
            //    Read cell value data
                ...
                foreach ($rowData as $rowDatum) {
                    switch ($rowDatum{0}) {
                        case 'C':
                        case 'X':
                            $column = mb_orig_substr($rowDatum, 1);
                            break;
                        case 'R':
                        case 'Y':
                            $row = mb_orig_substr($rowDatum, 1);
                            break;
                        case 'K':
                            $cellData = mb_orig_substr($rowDatum, 1);
                            break;
                        case 'E':
                            $cellDataFormula = '='.mb_orig_substr($rowDatum, 1);
                            ...
                            foreach ($temp as &$value) {
                                ...
                                if ($key = !$key) {
                                    ...
                                    foreach ($cellReferences as $cellReference) {
                                        ...
                                        $value = substr_replace($value, $A1CellReference, $cellReference[0][1], mb_orig_strlen($cellReference[0][0]));
                                    }
                                }
                            }
                            unset($value);
                ...
                foreach ($rowData as $rowDatum) {
                    switch ($rowDatum{0}) {
                        ...
                        case 'P':
                            ...
                            break;
                        case 'W':
                            list($startCol, $endCol, $columnWidth) = explode(' ', mb_orig_substr($rowDatum, 1));
                            break;
                        case 'S':
                            $styleSettings = mb_orig_substr($rowDatum, 1);
                            for ($i=0; $i<mb_orig_strlen($styleSettings); ++$i) {
