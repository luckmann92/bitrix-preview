...
class PHPExcel_Reader_Excel2003XML extends PHPExcel_Reader_Abstract implements PHPExcel_Reader_IReader
{
    ...
    public function canRead($pFilename)
    {
        ...
        foreach ($signature as $match) {
            ...
            if (mb_orig_strpos($data, $match) === false) {
                $valid = false;
                break;
            }
        }
        ...
        if (preg_match('/<?xml.*encoding=[\'"](.*?)[\'"].*?>/um', $data, $matches)) {
            $this->charSet = mb_orig_strtoupper($matches[1]);
        }
//        echo 'Character Set is ', $this->charSet,'<br />';
    ...
    {
        $styleAttributeValue = mb_orig_strtolower($styleAttributeValue);
        foreach ($styleList as $style) {
            if ($styleAttributeValue == mb_orig_strtolower($style)) {
...
//                                                $thisBorder['style'] = $borderStyleValue;
                                        break;
                                    case 'Position':
                                        $borderPosition = mb_orig_strtolower($borderStyleValue);
                                        break;
                                    case 'Color':
                                        $borderColour = mb_orig_substr($borderStyleValue, 1);
...
//                                echo $styleAttributeKey.' = '.$styleAttributeValue.'<br />';
                            ...
                            switch ($styleAttributeKey) {
                                ...
                                case 'Size':
                                    ...
                                    break;
                                case 'Color':
                                    $this->styles[$styleID]['font']['color']['rgb'] = mb_orig_substr($styleAttributeValue, 1);
                                    break;
                                case 'Bold':
...
//                                echo $styleAttributeKey.' = '.$styleAttributeValue.'<br />';
                            switch ($styleAttributeKey) {
                                case 'Color':
                                    $this->styles[$styleID]['fill']['color']['rgb'] = mb_orig_substr($styleAttributeValue, 1);
                                    break;
                            }
                        }
                        break;
...
//                                echo 'FORMULA<br />';
                                ...
                                if (mb_orig_substr($cellDataFormula, 0, 3) == 'of:') {
                                    $cellDataFormula = mb_orig_substr($cellDataFormula, 3);
...
//                            $annotation = str_replace('html:','',mb_orig_substr($node,49,-10));
//                            echo $annotation,'<br />';
