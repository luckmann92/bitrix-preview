...
//                        echo '<br />';
            ...
            if (isset($sheet->MergedRegions)) {
                foreach ($sheet->MergedRegions->Merge as $mergeCells) {
                    if (mb_orig_strpos($mergeCells, ':') !== false) {
        ...
        if (isset($gnmXML->Names)) {
            foreach ($gnmXML->Names->Name as $namedRange) {
                ...
                if (mb_orig_stripos($range, '#REF!') !== false) {
                    continue;
                }
    ...
    private static function parseGnumericColour($gnmColour)
    {
        ...
        $gnmR = mb_orig_substr(str_pad($gnmR, 4, '0', STR_PAD_RIGHT), 0, 2);
        $gnmG = mb_orig_substr(str_pad($gnmG, 4, '0', STR_PAD_RIGHT), 0, 2);
        $gnmB = mb_orig_substr(str_pad($gnmB, 4, '0', STR_PAD_RIGHT), 0, 2);
