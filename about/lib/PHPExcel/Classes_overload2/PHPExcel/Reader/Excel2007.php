...
//        echo '$c->f is ', $c->f, PHP_EOL;
        ...
        // Shared formula?
        if (isset($c->f['t']) && mb_orig_strtolower((string)$c->f['t']) == 'shared') {
...
//                echo 'Adjusted Formula is ', $value, PHP_EOL;
    ...
    {
        // Root-relative paths
        if (mb_orig_strpos($fileName, '//') !== false) {
            $fileName = mb_orig_substr($fileName, mb_orig_strpos($fileName, '//') + 1);
        }
        ...
        if ($contents === false) {
            $contents = $archive->getFromIndex(
                $archive->locateName(mb_orig_substr($fileName, 1), ZIPARCHIVE::FL_NOCASE)
            );
        }
    ...
    //                                    echo 'Value is ', $value, PHP_EOL;
                            ...
                            if (!$this->readDataOnly && $xmlSheet && $xmlSheet->conditionalFormatting) {
                                ...
                                foreach ($conditionals as $ref => $cfRules) {
                                    ...
                                    $cellBlocks = explode(' ', str_replace('$', '', mb_orig_strtoupper($ref)));
                            ...
                            if ($xmlSheet && $xmlSheet->autoFilter && !$this->readDataOnly) {
                                ...
                                if (mb_orig_strpos($autoFilterRange, ':') !== false) {
                            ...
                            if ($xmlSheet && $xmlSheet->mergeCells && $xmlSheet->mergeCells->mergeCell && !$this->readDataOnly) {
                                foreach ($xmlSheet->mergeCells->mergeCell as $mergeCell) {
                                    ...
                                    if (mb_orig_strpos($mergeRef, ':') !== false) {
                            ...
                            if ($xmlSheet && $xmlSheet->dataValidations && !$this->readDataOnly) {
                                foreach ($xmlSheet->dataValidations->dataValidation as $dataValidation) {
                                    ...
                                    $range = mb_orig_strtoupper($dataValidation["sqref"]);
                                ...
                                foreach ($vmlComments as $relName => $relPath) {
                                    ...
                                    foreach ($shapes as $shape) {
                                        ...
                                        if (isset($shape['style'])) {
                                            ...
                                            $fillColor    = mb_orig_strtoupper(mb_orig_substr((string)$shape['fillcolor'], 1));
                                            $column       = null;
                                            $row          = null;
                            ...
                            if ($xmlWorkbook->definedNames) {
                                foreach ($xmlWorkbook->definedNames->definedName as $definedName) {
                                    ...
                                    if (($spos = mb_orig_strpos($extractedRange, '!')) !== false) {
                                        $extractedRange = mb_orig_substr($extractedRange, 0, $spos).str_replace('$', '', mb_orig_substr($extractedRange, $spos));
                                    } else {
                                        $extractedRange = str_replace('$', '', $extractedRange);
                                    }

                                    // Valid range?
                                    if (mb_orig_stripos((string)$definedName, '#REF!') !== false || $extractedRange == '') {
                                        continue;
                                    }
                                        ...
                                        switch ((string)$definedName['name']) {
                                            ...
                                            case '_xlnm.Print_Area':
                                                ...
                                                foreach ($rangeSets as $rangeSet) {
                                                    ...
                                                    if (mb_orig_strpos($rangeSet, ':') === false) {
                        ...
                        if ($xmlWorkbook->definedNames) {
                            foreach ($xmlWorkbook->definedNames->definedName as $definedName) {
                                ...
                                if ((string)$definedName['localSheetId'] != '') {
                                    ...
                                    $locatedSheet = null;
                                    $extractedSheetName = '';
                                    if (mb_orig_strpos((string)$definedName, '!') !== false) {
                                        // Extract sheet name
...
//        }
        ...
        if (isset($style->font)) {
            ...
            if (isset($style->font->vertAlign) && isset($style->font->vertAlign["val"])) {
                $vertAlign = mb_orig_strtolower((string)$style->font->vertAlign["val"]);
                ...
                foreach ($is->r as $run) {
                    if (!isset($run->rPr)) {
                        ...
                        if (isset($run->rPr->vertAlign) && isset($run->rPr->vertAlign["val"])) {
                            $vertAlign = mb_orig_strtolower((string)$run->rPr->vertAlign["val"]);
                            if ($vertAlign == 'superscript') {
    ...
    private static function toCSSArray($style)
    {
        ...
        foreach ($temp as $item) {
            ...
            if (mb_orig_strpos($item[1], 'px') !== false) {
                $item[1] = str_replace('px', '', $item[1]);
            }
            if (mb_orig_strpos($item[1], 'pt') !== false) {
                $item[1] = str_replace('pt', '', $item[1]);
                $item[1] = PHPExcel_Shared_Font::fontSizeToPixels($item[1]);
            }
            if (mb_orig_strpos($item[1], 'in') !== false) {
                $item[1] = str_replace('in', '', $item[1]);
                $item[1] = PHPExcel_Shared_Font::inchSizeToPixels($item[1]);
            }
            if (mb_orig_strpos($item[1], 'cm') !== false) {
