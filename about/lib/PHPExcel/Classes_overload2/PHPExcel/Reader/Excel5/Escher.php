...
class PHPExcel_Reader_Excel5_Escher
{
    ...
    private function readDefault()
    {
        ...
        $recordData = mb_orig_substr($this->data, $this->pos + 8, $length);
    ...
    private function readBSE()
    {
        ...
        $rgbUid = mb_orig_substr($recordData, 2, 16);
        ...
        $nameData = mb_orig_substr($recordData, 36, $cbName);

        // offset: 36 + $cbName, size: var; the BLIP data
        $blipData = mb_orig_substr($recordData, 36 + $cbName);
    ...
    private function readBlipJPEG()
    {
        ...
        $rgbUid1 = mb_orig_substr($recordData, 0, 16);
        $pos += 16;

        // offset: 16; size: 16; rgbUid2 (MD4 digest), only if $recInstance = 0x46B or 0x6E3
        if (in_array($recInstance, array(0x046B, 0x06E3))) {
            $rgbUid2 = mb_orig_substr($recordData, 16, 16);
            $pos += 16;
        }
        ...
        $data = mb_orig_substr($recordData, $pos);
    ...
    {
        $splicedComplexData = mb_orig_substr($data, 6 * $n);

        // loop through property-value pairs
        for ($i = 0; $i < $n; ++$i) {
            // read 6 bytes at a time
            $fopte = mb_orig_substr($data, 6 * $i, 6);
            ...
            if ($opidFComplex) {
                $complexData = mb_orig_substr($splicedComplexData, 0, $op);
                $splicedComplexData = mb_orig_substr($splicedComplexData, $op);
