...
class PHPExcel_ReferenceHelper
{
    ...
    {
        return strcasecmp(mb_orig_strlen($a) . $a, mb_orig_strlen($b) . $b);
    }
    ...
    {
        return 1 - strcasecmp(mb_orig_strlen($a) . $a, mb_orig_strlen($b) . $b);
    }
    ...
    public static function cellSort($a, $b)
    {
        ...
        if ($ar == $br) {
            return strcasecmp(mb_orig_strlen($ac) . $ac, mb_orig_strlen($bc) . $bc);
        }
        return ($ar < $br) ? -1 : 1;
    }
    ...
    public static function cellReverseSort($a, $b)
    {
        ...
        if ($ar == $br) {
            return 1 - strcasecmp(mb_orig_strlen($ac) . $ac, mb_orig_strlen($bc) . $bc);
        }
        return ($ar < $br) ? 1 : -1;
    }
            ...
            if ($i = !$i) {
                ...
                if ($matchCount > 0) {
                    foreach ($matches as $match) {
                        ...
                        $modified3 = mb_orig_substr($this->updateCellReference('$A'.$match[3], $pBefore, $pNumCols, $pNumRows), 2);
                        $modified4 = mb_orig_substr($this->updateCellReference('$A'.$match[4], $pBefore, $pNumCols, $pNumRows), 2);
                ...
                if ($matchCount > 0) {
                    foreach ($matches as $match) {
                        $fromString = ($match[2] > '') ? $match[2].'!' : '';
                        $fromString .= $match[3].':'.$match[4];
                        $modified3 = mb_orig_substr($this->updateCellReference($match[3].'$1', $pBefore, $pNumCols, $pNumRows), 0, -2);
                        $modified4 = mb_orig_substr($this->updateCellReference($match[4].'$1', $pBefore, $pNumCols, $pNumRows), 0, -2);
    ...
    public function updateCellReference($pCellRange = 'A1', $pBefore = 'A1', $pNumCols = 0, $pNumRows = 0)
    {
        ...
        if (mb_orig_strpos($pCellRange, "!") !== false) {
            return $pCellRange;
        // Is it a range or a single cell?
        } elseif (mb_orig_strpos($pCellRange, ':') === false && mb_orig_strpos($pCellRange, ',') === false) {
            // Single cell
        ...
        foreach ($pPhpExcel->getWorksheetIterator() as $sheet) {
            foreach ($sheet->getCellCollection(false) as $cellID) {
                ...
                if (($cell !== null) && ($cell->getDataType() == PHPExcel_Cell_DataType::TYPE_FORMULA)) {
                    ...
                    if (mb_orig_strpos($formula, $oldName) !== false) {
    ...
    {
        if (mb_orig_strpos($pCellRange, ':') !== false || mb_orig_strpos($pCellRange, ',') !== false) {
            // Update range
    ...
    {
        if (mb_orig_strpos($pCellReference, ':') === false && mb_orig_strpos($pCellReference, ',') === false) {
            // Get coordinates of $pBefore
            list($beforeColumn, $beforeRow) = PHPExcel_Cell::coordinateFromString($pBefore);
