...
class PHPExcel_Calculation
{
    ...
    private static function loadLocales()
    {
        ...
        foreach (glob($localeFileDirectory.'/*', GLOB_ONLYDIR) as $filename) {
            $filename = mb_orig_substr($filename, mb_orig_strlen($localeFileDirectory)+1);
            if ($filename != 'en') {
    ...
    public function setLocale($locale = 'en_us')
    {
        ...
        $language = $locale = mb_orig_strtolower($locale);
        if (mb_orig_strpos($locale, '_') !== false) {
        ...
        if (in_array($language, self::$validLocaleLanguages)) {
            ...
            if ($locale != 'en_us') {
                ...
                foreach ($localeFunctions as $localeFunction) {
                    ...
                    if (mb_orig_strpos($localeFunction, '=') !== false) {
                ...
                if (file_exists($configFile)) {
                    ...
                    foreach ($localeSettings as $localeSetting) {
                        ...
                        if (mb_orig_strpos($localeSetting, '=') !== false) {
                            list($settingName, $settingValue) = explode('=', $localeSetting);
                            $settingName = mb_orig_strtoupper(trim($settingName));
                            switch ($settingName) {
    ...
    private static function translateFormula($from, $to, $formula, $fromSeparator, $toSeparator)
    {
        ...
        if (self::$localeLanguage !== 'en_us') {
            ...
            if (mb_orig_strpos($formula, '"') !== false) {
    ...
    {
        if (is_string($value)) {
            if ((isset($value{0})) && ($value{0} == '"') && (mb_orig_substr($value, -1) == '"')) {
                return mb_orig_substr($value, 1, -1);
            }
        //    Convert numeric errors to NaN error
        } elseif ((is_float($value)) && ((is_nan($value)) || (is_infinite($value)))) {
            return PHPExcel_Calculation_Functions::NaN();
    ...
    public function parseFormula($formula)
    {
        ...
        $formula = ltrim(mb_orig_substr($formula, 1));
        if (!isset($formula{0})) {
            return array();
        }
        ...
        if (mb_orig_strpos($formula, '{') !== false) {
            ...
            if (mb_orig_strpos($formula, '"') !== false) {
                ...
                foreach ($temp as &$value) {
                    //    Only count/replace in alternating array entries
                    if ($i = !$i) {
                        $openCount += mb_orig_substr_count($value, '{');
                        $closeCount += mb_orig_substr_count($value, '}');
                ...
                //    If there's no quoted strings, then we do a simple count/replace
                $openCount = mb_orig_substr_count($formula, '{');
                $closeCount = mb_orig_substr_count($formula, '}');
    ...
    private function _parseFormula($formula, PHPExcel_Cell $pCell = null)
    {
        ...
        while (true) {
//echo 'Assessing Expression '.mb_orig_substr($formula, $index), PHP_EOL;
            $opCharacter = $formula{$index};    //    Get the first character of the value at the current index position
//echo 'Initial character of expression block is '.$opCharacter, PHP_EOL;
            if ((isset(self::$comparisonOperators[$opCharacter])) && (mb_orig_strlen($formula) > $index) && (isset(self::$comparisonOperators[$formula{$index+1}]))) {
                $opCharacter .= $formula{++$index};
//echo 'Initial character of expression block is comparison operator '.$opCharacter.PHP_EOL;
            }

            //    Find out if we're currently at the beginning of a number, variable, cell reference, function, parenthesis or operand
            $isOperandOrFunction = preg_match($regexpMatchString, mb_orig_substr($formula, $index), $match);
...
//                    echo 'Element '.$val.' is a Function<br />';
                    if (isset(self::$PHPExcelFunctions[mb_orig_strtoupper($matches[1])]) || isset(self::$controlFunctions[mb_orig_strtoupper($matches[1])])) {    // it's a function
                        $stack->push('Function', mb_orig_strtoupper($val));
                        $ax = preg_match('/^\s*(\s*\))/ui', mb_orig_substr($formula, $index+$length), $amatch);
                        if ($ax) {
                            $stack->push('Operand Count for Function '.mb_orig_strtoupper($val).')', 0);
                            $expectingOperator = true;
                        } else {
                            $stack->push('Operand Count for Function '.mb_orig_strtoupper($val).')', 1);
                            $expectingOperator = false;
                        }
                        $stack->push('Brace', '(');
...
//                    echo 'Element is a Variable, Constant, String, Number or Boolean<br />';
                    ...
                    if ($testPrevOp['value'] == ':') {
                        ...
                        $rangeWS1 = '';
                        if (mb_orig_strpos('!', $startRowColRef) !== false) {
                        ...
                        if ((is_integer($startRowColRef)) && (ctype_digit($val)) &&
                            ...
                            (mb_orig_strlen($startRowColRef) <= 3) && (mb_orig_strlen($val) <= 3)) {
                            //    Column range
                            $endRowColRef = ($pCellParent !== null) ? $pCellParent->getHighestRow() : 1048576;        //    Max 1,048,576 rows for Excel2007
                            $output[count($output)-1]['value'] = $rangeWS1.mb_orig_strtoupper($startRowColRef).'1';
...
//                        echo 'Element is a Number<br />';
                        if ((mb_orig_strpos($val, '.') !== false) || (mb_orig_stripos($val, 'e') !== false) || ($val > PHP_INT_MAX) || ($val < -PHP_INT_MAX)) {
...
//                            echo 'Casting '.$val.' to integer<br />';
                    ...
                    } elseif (isset(self::$excelConstants[trim(mb_orig_strtoupper($val))])) {
                        $excelConstant = trim(mb_orig_strtoupper($val));
//                        echo 'Element '.$excelConstant.' is an Excel Constant<br />';
                        $val = self::$excelConstants[$excelConstant];
                    } elseif (($localeConstant = array_search(trim(mb_orig_strtoupper($val)), self::$localeBoolean)) !== false) {
//                        echo 'Element '.$localeConstant.' is an Excel Constant<br />';
            ...
            if ($index == mb_orig_strlen($formula)) {
...
//                echo 'Possible Intersect Operator<br />';
                if (($expectingOperator) && (preg_match('/^'.self::CALCULATION_REGEXP_CELLREF.'.*/Ui', mb_orig_substr($formula, $index), $match)) &&
...
//                echo 'Token is a binary operator<br />';
                ...
                switch ($token) {
                    ...
                    case ':':            //    Range
                        $sheet1 = $sheet2 = '';
                        if (mb_orig_strpos($operand1Data['reference'], '!') !== false) {
                            list($sheet1, $operand1Data['reference']) = explode('!', $operand1Data['reference']);
                        } else {
                            $sheet1 = ($pCellParent !== null) ? $pCellWorksheet->getTitle() : '';
                        }
                        if (mb_orig_strpos($operand2Data['reference'], '!') !== false) {
...
//                        We can't access the range, so return a REF error
                        ...
                        if ($matches[2] > '') {
                            ...
                            if ((mb_orig_strpos($matches[2], '[') !== false) || (mb_orig_strpos($matches[2], ']') !== false)) {
...
//                    } else {
                    ...
                    if (mb_orig_strpos($functionCall, '::') !== false) {
                ...
                if (isset(self::$excelConstants[mb_orig_strtoupper($token)])) {
                    $excelConstant = mb_orig_strtoupper($token);
...
//        }
    ...
    private function executeBinaryComparisonOperation($cellID, $operand1, $operand2, $operation, &$stack, $recursingArrays = false)
    {
        ...
        if (PHPExcel_Calculation_Functions::getCompatibilityMode() != PHPExcel_Calculation_Functions::COMPATIBILITY_OPENOFFICE) {
            if (is_string($operand1)) {
                $operand1 = mb_orig_strtoupper($operand1);
            }
            if (is_string($operand2)) {
                $operand2 = mb_orig_strtoupper($operand2);
            }
        }
    ...
    private function executeNumericBinaryOperation($cellID, $operand1, $operand2, $operation, $matrixFunction, &$stack)
    {
        ...
        if ((is_array($operand1)) || (is_array($operand2))) {
            ...
            if ((PHPExcel_Calculation_Functions::getCompatibilityMode() != PHPExcel_Calculation_Functions::COMPATIBILITY_OPENOFFICE) &&
                ((is_string($operand1) && !is_numeric($operand1) && mb_orig_strlen($operand1)>0) ||
                 (is_string($operand2) && !is_numeric($operand2) && mb_orig_strlen($operand2)>0))) {
...
//            echo 'Range reference is '.$pRange.PHP_EOL;
            if (mb_orig_strpos($pRange, '!') !== false) {
...
//            echo '<br />';
    ...
    {
        $pFunction = mb_orig_strtoupper($pFunction);
