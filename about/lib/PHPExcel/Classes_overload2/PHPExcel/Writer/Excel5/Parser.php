...
class PHPExcel_Writer_Excel5_Parser
{
    ...
    private function convertString($string)
    {
        ...
        $string = mb_orig_substr($string, 1, mb_orig_strlen($string) - 2);
        if (mb_orig_strlen($string) > 255) {
...
//        }
    ...
    private function cellToRowcol($cell)
    {
        ...
        $expn = mb_orig_strlen($col_ref) - 1;
        $col  = 0;
        $col_ref_length = mb_orig_strlen($col_ref);
    ...
    private function advance()
    {
        ...
        $formula_length = mb_orig_strlen($this->formula);
        // eat up white spaces
        if ($i < $formula_length) {
        ...
        while ($i < $formula_length) {
            ...
            if ($this->match($token) != '') {
                //if ($i < mb_orig_strlen($this->formula) - 1) {
                //    $this->lookAhead = $this->formula{$i+1};
                //}
        ...
        switch ($token) {
            ...
            default:
                ...
                if (preg_match('/^\$?[A-Ia-i]?[A-Za-z]\$?[0-9]+$/', $token) and !preg_match("/[0-9]/", $this->lookAhead) and ($this->lookAhead != ':') and ($this->lookAhead != '.') and ($this->lookAhead != '!')) {
                    ...
                    return $token;
                } elseif (preg_match("/\"([^\"]|\"\"){0,255}\"/", $token) and $this->lookAhead != '"' and (mb_orig_substr_count($token, '"')%2 == 0)) {
    ...
    private function func()
    {
        ...
        $function = mb_orig_strtoupper($this->currentToken);
