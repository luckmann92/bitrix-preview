...
class PHPExcel_Writer_Excel5_BIFFwriter
{
    ...
    /**
     * The size of the data in bytes. Should be the same as mb_orig_strlen($this->_data)
     * @var integer
     */
    public $_datasize;
...
//        $this->limit      = 8224;
    ...
    {
        if (mb_orig_strlen($data) - 4 > $this->limit) {
            $data = $this->addContinue($data);
        }
        $this->_data     .= $data;
        $this->_datasize += mb_orig_strlen($data);
    }
    ...
    private function addContinue($data)
    {
        ...
        $tmp = mb_orig_substr($data, 0, 2) . pack("v", $limit) . mb_orig_substr($data, 4, $limit);

        $header = pack("vv", $record, $limit);  // Headers for continue records

        // Retrieve chunks of 2080/8224 bytes +4 for the header.
        $data_length = mb_orig_strlen($data);
        for ($i = $limit + 4; $i < ($data_length - $limit); $i += $limit) {
            $tmp .= $header;
            $tmp .= mb_orig_substr($data, $i, $limit);
        }

        // Retrieve the last chunk of data
        $header  = pack("vv", $record, mb_orig_strlen($data) - $i);
        $tmp    .= $header;
        $tmp    .= mb_orig_substr($data, $i, mb_orig_strlen($data) - $i);
