...
class PHPExcel_Writer_Excel5_Workbook extends PHPExcel_Writer_Excel5_BIFFwriter
{
        ...
        if (!isset($this->colors[$rgb])) {
            if (count($this->colors) < 57) {
                ...
                $this->palette[$colorIndex] =
                    array(
                        hexdec(mb_orig_substr($rgb, 0, 2)),
                        hexdec(mb_orig_substr($rgb, 2, 2)),
                        hexdec(mb_orig_substr($rgb, 4)),
                        0
                    );
    ...
    private function calcSheetOffsets()
    {
        ...
        foreach ($this->phpExcel->getWorksheetIterator() as $sheet) {
            $offset += $boundsheet_length + mb_orig_strlen(PHPExcel_Shared_String::UTF8toBIFF8UnicodeShort($sheet->getTitle()));
        }
            ...
            foreach ($namedRanges as $namedRange) {
                ...
                try {
                    ...
                    if (isset($formulaData{0}) and ($formulaData{0} == "\x7A" or $formulaData{0} == "\x5A")) {
                        $formulaData = "\x3A" . mb_orig_substr($formulaData, 1);
                    }
    ...
    private function writeDefinedNameBiff8($name, $formulaData, $sheetIndex = 0, $isBuiltIn = false)
    {
        ...
        $name = mb_orig_substr(PHPExcel_Shared_String::UTF8toBIFF8UnicodeLong($name), 2);

        // size of the formula (in bytes)
        $sz = mb_orig_strlen($formulaData);
    ...
    private function writeNumberFormat($format, $ifmt)
    {
        ...
        $length = 2 + mb_orig_strlen($numberFormatString);    // Number of bytes to follow
    ...
    {
        $record = 0x0017;                     // Record identifier
        $length = 0x02 + mb_orig_strlen($sheetname);  // Number of bytes to follow

        $cch    = mb_orig_strlen($sheetname);         // Length of sheet name
    ...
    private function writeSharedStringsTable()
    {
        ...
        foreach (array_keys($this->stringTable) as $string) {
            ...
            while ($finished === false) {
                ...
                if (mb_orig_strlen($recordData) + mb_orig_strlen($string) <= $continue_limit) {
                    ...
                    $space_remaining = $continue_limit - mb_orig_strlen($recordData);
                    ...
                    // 2. space remaining is greater than or equal to minimum space needed
                        ...
                        if ($encoding == 1 && (mb_orig_strlen($string) - $space_remaining) % 2 == 1) {
                            --$effective_space_remaining;
                        }

                        // one block fininshed, store the block data
                        $recordData .= mb_orig_substr($string, 0, $effective_space_remaining);

                        $string = mb_orig_substr($string, $effective_space_remaining); // for next cycle in while loop
                        $recordDatas[] = $recordData;
        ...
        if (mb_orig_strlen($recordData) > 0) {
            $recordDatas[] = $recordData;
        }
        ...
        foreach ($recordDatas as $i => $recordData) {
            ...
            $header = pack("vv", $record, mb_orig_strlen($recordData));
