...
//            if ($row + 1 > 65536 or $column + 1 > 256) {
        ...
        foreach ($phpSheet->getHyperLinkCollection() as $coordinate => $hyperlink) {
            ...
            if (mb_orig_strpos($url, 'sheet://') !== false) {
    ...
    private function writeLabel($row, $col, $str, $xfIndex)
    {
        ...
        if ($strlen > $this->xlsStringMaxLength) { // LABEL must be < 255 chars
            $str    = mb_orig_substr($str, 0, $this->xlsStringMaxLength);
    ...
    {
        $note_length    = mb_orig_strlen($note);
        ...
        $this->append($header . $data . mb_orig_substr($note, 0, 2048));

        for ($i = $max_length; $i < $note_length; $i += $max_length) {
            $chunk  = mb_orig_substr($note, $i, $max_length);
            $length = 0x0006 + mb_orig_strlen($chunk);
            $header = pack("vv", $record, $length);
            $data   = pack("vvv", -1, 0, mb_orig_strlen($chunk));
    ...
    private function writeFormula($row, $col, $formula, $xfIndex, $calculatedValue)
    {
        ...
        if ($formula{0} == '=') {
            $formula = mb_orig_substr($formula, 1);
        } else {
            // Error handling
        ...
        try {
            ...
            $formlen    = mb_orig_strlen($formula);    // Length of the binary string
    ...
    public function writeUrlWeb($row1, $col1, $row2, $col2, $url)
    {
        ...
        $url_len     = pack("V", mb_orig_strlen($url));

        // Calculate the data length
        $length      = 0x34 + mb_orig_strlen($url);
    ...
    public function writeUrlInternal($row1, $col1, $row2, $col2, $url)
    {
        ...
        // Calculate the data length
        $length      = 0x24 + mb_orig_strlen($url);
    ...
    public function writeUrlExternal($row1, $col1, $row2, $col2, $url)
    {
        ...
        $dir_short_len = pack("V", mb_orig_strlen($dir_short));
        $dir_long_len  = pack("V", mb_orig_strlen($dir_long));
        $stream_len    = pack("V", 0); //mb_orig_strlen($dir_long) + 0x06);
    ...
    private function writeMergedCells()
    {
        ...
        foreach ($mergeCells as $mergeCell) {
            ...
            if ($j == $maxCountMergeCellsPerRecord or $i == $countMergeCells) {
                ...
                $length = mb_orig_strlen($recordData);
                $header = pack('vv', $record, $length);
    ...
    private function writeExternsheet($sheetname)
    {
        ...
        if ($this->phpSheet->getTitle() == $sheetname) {
            ...
            $length    = 0x02 + mb_orig_strlen($sheetname);
            $cch       = mb_orig_strlen($sheetname);
    ...
    private function writeHeader()
    {
        ...
        if (mb_orig_strlen($this->phpSheet->getHeaderFooter()->getOddHeader()) <= 255) {
    ...
    private function writeFooter()
    {
        ...
        /* removing for now
        // need to fix character count (multibyte!)
        if (mb_orig_strlen($this->phpSheet->getHeaderFooter()->getOddFooter()) <= 255) {
    ...
    public function processBitmapGd($image)
    {
        ...
        return array($width, $height, mb_orig_strlen($data), $data);
    }
    ...
    public function processBitmap($bitmap)
    {
        ...
        //
        $size_array   = unpack("Vsa", mb_orig_substr($data, 0, 4));
        $size   = $size_array['sa'];
        $data   = mb_orig_substr($data, 4);
        $size  -= 0x36; // Subtract size of bitmap header.
        $size  += 0x0C; // Add size of BIFF header.

        // Remove bitmap data: reserved, offset, header length.
        $data = mb_orig_substr($data, 12);

        // Read and remove the bitmap width and height. Verify the sizes.
        $width_and_height = unpack("V2", mb_orig_substr($data, 0, 8));
        $width  = $width_and_height[1];
        $height = $width_and_height[2];
        $data   = mb_orig_substr($data, 8);
        if ($width > 0xFFFF) {
        ...
        $planes_and_bitcount = unpack("v2", mb_orig_substr($data, 0, 4));
        $data = mb_orig_substr($data, 4);
        ...
        $compression = unpack("Vcomp", mb_orig_substr($data, 0, 4));
        $data = mb_orig_substr($data, 4);
    ...
    private function writeMsoDrawing()
    {
        ...
        if (isset($this->escher)) {
            ...
            for ($i = 1; $i <= $nm; ++$i) {
                ...
                $dataChunk = mb_orig_substr($data, $spOffsets[$i -1], $spOffsets[$i] - $spOffsets[$i - 1]);

                $length = mb_orig_strlen($dataChunk);
                $header = pack("vv", $record, $length);
    ...
    private function writeCFRule(PHPExcel_Style_Conditional $conditional)
    {
        ...
        $header      = pack('vv', $record, mb_orig_strlen($data));
        $this->append($header . $data);
    }
