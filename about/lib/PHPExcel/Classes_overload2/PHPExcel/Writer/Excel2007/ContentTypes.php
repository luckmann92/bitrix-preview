    ...
    public function writeContentTypes(PHPExcel $pPHPExcel = null, $includeCharts = false)
    {
        ...
        for ($i = 0; $i < $mediaCount; ++$i) {
            ...
            if ($this->getParentWriter()->getDrawingHashTable()->getByIndex($i) instanceof PHPExcel_Worksheet_Drawing) {
                $extension = mb_orig_strtolower($this->getParentWriter()->getDrawingHashTable()->getByIndex($i)->getExtension());
                ...
                $extension = mb_orig_strtolower($this->getParentWriter()->getDrawingHashTable()->getByIndex($i)->getMimeType());
        ...
        for ($i = 0; $i < $sheetCount; ++$i) {
            if (count($pPHPExcel->getSheet()->getHeaderFooter()->getImages()) > 0) {
                foreach ($pPHPExcel->getSheet()->getHeaderFooter()->getImages() as $image) {
                    if (!isset( $aMediaContentTypes[mb_orig_strtolower($image->getExtension())])) {
                        $aMediaContentTypes[mb_orig_strtolower($image->getExtension())] = $this->getImageMimeType($image->getPath());

                        $this->writeDefaultContentType($objWriter, mb_orig_strtolower($image->getExtension()), $aMediaContentTypes[mb_orig_strtolower($image->getExtension())]);
                    }
                }
            }
        }
