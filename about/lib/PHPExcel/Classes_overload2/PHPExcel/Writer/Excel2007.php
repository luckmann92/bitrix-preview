...
class PHPExcel_Writer_Excel2007 extends PHPExcel_Writer_Abstract implements PHPExcel_Writer_IWriter
{
    ...
    {
        if ($pPartName != '' && isset($this->writerParts[mb_orig_strtolower($pPartName)])) {
            return $this->writerParts[mb_orig_strtolower($pPartName)];
        } else {
            return null;
        }
    }
    ...
    public function save($pFilename = null)
    {
        if ($this->spreadSheet !== null) {
            ...
            if (mb_orig_strtolower($pFilename) == 'php://output' || mb_orig_strtolower($pFilename) == 'php://stdout') {
            ...
            for ($i = 0; $i < $this->getDrawingHashTable()->count(); ++$i) {
                if ($this->getDrawingHashTable()->getByIndex($i) instanceof PHPExcel_Worksheet_Drawing) {
                    ...
                    if (mb_orig_strpos($imagePath, 'zip://') !== false) {
                        $imagePath = mb_orig_substr($imagePath, 6);
