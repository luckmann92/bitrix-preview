...
class PHPExcel_Worksheet implements PHPExcel_IComparable
{
   ...
   /**
    ...
    {
        $cell = $this->getCell(mb_orig_strtoupper($pCoordinate))->setValue($pValue);
    ...
    {
        // Set value
        $cell = $this->getCell(mb_orig_strtoupper($pCoordinate))->setValueExplicit($pValue, $pDataType);
        return ($returnCell) ? $cell : $this;
    }
    ...
    public function getCell($pCoordinate = 'A1', $createIfNotExists = true)
    {
        ...
        if ($this->cellCollection->isDataSet(mb_orig_strtoupper($pCoordinate))) {
            return $this->cellCollection->getCacheData($pCoordinate);
        }

        // Worksheet reference?
        if (mb_orig_strpos($pCoordinate, '!') !== false) {
            $worksheetReference = PHPExcel_Worksheet::extractSheetTitle($pCoordinate, true);
            return $this->parent->getSheetByName($worksheetReference[0])->getCell(mb_orig_strtoupper($worksheetReference[1]), $createIfNotExists);
        }
        ...
        $pCoordinate = mb_orig_strtoupper($pCoordinate);

        if (mb_orig_strpos($pCoordinate, ':') !== false || mb_orig_strpos($pCoordinate, ',') !== false) {
            throw new PHPExcel_Exception('Cell coordinate can not be a range of cells.');
        } elseif (mb_orig_strpos($pCoordinate, '$') !== false) {
    ...
    public function cellExists($pCoordinate = 'A1')
    {
       // Worksheet reference?
        if (mb_orig_strpos($pCoordinate, '!') !== false) {
            $worksheetReference = PHPExcel_Worksheet::extractSheetTitle($pCoordinate, true);
            return $this->parent->getSheetByName($worksheetReference[0])->cellExists(mb_orig_strtoupper($worksheetReference[1]));
        }
    ...
    {
        // Uppercase coordinate
        $pColumn = mb_orig_strtoupper($pColumn);
    ...
    public function getStyle($pCellCoordinate = 'A1')
    {
        ...
        $this->setSelectedCells(mb_orig_strtoupper($pCellCoordinate));
    ...
    {
        if (isset($this->conditionalStylesCollection[mb_orig_strtoupper($pCoordinate)])) {
            return true;
        }
        return false;
    }
    ...
    {
        unset($this->conditionalStylesCollection[mb_orig_strtoupper($pCoordinate)]);
        return $this;
    }
    ...
    {
        $this->conditionalStylesCollection[mb_orig_strtoupper($pCoordinate)] = $pValue;
        return $this;
    }
    ...
    {
        // Uppercase coordinate
        $pCell = mb_orig_strtoupper($pCell);
    ...
    {
        // Uppercase coordinate
        $pRange = mb_orig_strtoupper($pRange);
    ...
    {
        $pRange = mb_orig_strtoupper($pValue);
        if (is_string($pValue)) {
    ...
    public function freezePane($pCell = '')
    {
        // Uppercase coordinate
        $pCell = mb_orig_strtoupper($pCell);
        if (mb_orig_strpos($pCell, ':') === false && mb_orig_strpos($pCell, ',') === false) {
    ...
    */
    ...
    {
        // Uppercase coordinate
        $pCellCoordinate = mb_orig_strtoupper($pCellCoordinate);

        if (mb_orig_strpos($pCellCoordinate, ':') !== false || mb_orig_strpos($pCellCoordinate, ',') !== false) {
            throw new PHPExcel_Exception('Cell coordinate string can not be a range of cells.');
        } elseif (mb_orig_strpos($pCellCoordinate, '$') !== false) {
...
//
    ...
    public static function extractSheetTitle($pRange, $returnRange = false)
    {
        ...
        if (($sep = mb_orig_strpos($pRange, '!')) === false) {
            return '';
        }

        if ($returnRange) {
            return array(trim(mb_orig_substr($pRange, 0, $sep), "'"), mb_orig_substr($pRange, $sep + 1));
        }

        return mb_orig_substr($pRange, $sep + 1);
    }
