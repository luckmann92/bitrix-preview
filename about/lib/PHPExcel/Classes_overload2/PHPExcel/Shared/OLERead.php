...
class PHPExcel_Shared_OLERead
{
    ...
    public function read($sFileName)
    {
        ...
        for ($i = 0; $i < $this->numBigBlockDepotBlocks; ++$i) {
            ...
            $this->bigBlockChain .= mb_orig_substr($this->data, $pos, 4*$bbs);
            $pos += 4*$bbs;
        }
        ...
        while ($sbdBlock != -2) {
            ...
            $this->smallBlockChain .= mb_orig_substr($this->data, $pos, 4*$bbs);
            $pos += 4*$bbs;
    ...
    public function getStream($stream)
    {
        ...
        if ($this->props[$stream]['size'] < self::SMALL_BLOCK_THRESHOLD) {
            ...
            while ($block != -2) {
                ...
                $streamData .= mb_orig_substr($rootdata, $pos, self::SMALL_BLOCK_SIZE);
            ...
            while ($block != -2) {
                ...
                $streamData .= mb_orig_substr($this->data, $pos, self::BIG_BLOCK_SIZE);
    ...
    private function _readData($bl)
    {
        ...
        while ($block != -2) {
            $pos = ($block + 1) * self::BIG_BLOCK_SIZE;
            $data .= mb_orig_substr($this->data, $pos, self::BIG_BLOCK_SIZE);
            $block = self::getInt4d($this->bigBlockChain, $block*4);
        }
        return $data;
    }
    ...
    private function readPropertySets()
    {
        ...
        $entryLen = mb_orig_strlen($this->entry);
        while ($offset < $entryLen) {
            // entry data (128 bytes)
            $d = mb_orig_substr($this->entry, $offset, self::PROPERTY_STORAGE_BLOCK_SIZE);
            ...
            $name = str_replace("\x00", "", mb_orig_substr($d, 0, $nameSize));
            ...
            $upName = mb_orig_strtoupper($name);
