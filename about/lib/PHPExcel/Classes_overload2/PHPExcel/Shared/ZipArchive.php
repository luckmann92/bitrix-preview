...
class PHPExcel_Shared_ZipArchive
{
    ...
    {
        $fileName = mb_orig_strtolower($fileName);

        $list = $this->zip->listContent();
        $listCount = count($list);
        $index = -1;
        for ($i = 0; $i < $listCount; ++$i) {
            if (mb_orig_strtolower($list[$i]["filename"]) == $fileName ||
                mb_orig_strtolower($list[$i]["stored_filename"]) == $fileName) {
                $index = $i;
                break;
            }
        }
    ...
    public function getFromName($fileName)
    {
        ...
        if ($index !== false) {
            ...
            $fileName = mb_orig_substr($fileName, 1);
            $index = $this->locateName($fileName);
            if ($index === false) {
                return false;
            }
