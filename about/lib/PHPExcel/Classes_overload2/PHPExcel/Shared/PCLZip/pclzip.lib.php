...
class PclZip
{
        ...
        if ($v_size > 0) {
            ...
            if ((is_integer($v_arg_list[0])) && ($v_arg_list[0] > 77000)) {
                ...
                if (isset($v_options[PCLZIP_OPT_ADD_PATH])) {
                    ...
                    if ((mb_orig_strlen($v_path) > 0) && (mb_orig_substr($v_path, -1) != '/')) {
                        $v_path .= '/';
                    }
    ...
    public function privOptionDefaultThreshold(&$p_options)
    {
        ...
        $last = mb_orig_strtolower(mb_orig_substr($v_memory_limit, -1));
    ...
    public function privAddFile($p_filedescr, &$p_header, &$p_options)
    {
        ...
        */
        ...
        $p_header['filename_len'] = mb_orig_strlen($p_filename);
    ...
    // TBC : Removed        $p_header['stored_filename'] = $v_stored_filename;
        ...
        if ($p_filedescr['type']=='file') {
            ...
            $p_header['external'] = 0x00000000;
            $p_header['size'] = mb_orig_strlen($p_filedescr['content']);
        }
        ...
        if (isset($p_filedescr['comment'])) {
            $p_header['comment_len'] = mb_orig_strlen($p_filedescr['comment']);
    ...
    //            eval('$v_result = '.$p_options[PCLZIP_CB_PRE_ADD].'(PCLZIP_CB_PRE_ADD, $v_local_header);');
        ...
        if (mb_orig_strlen($p_header['stored_filename']) > 0xFF) {
            ...
            if ($p_filedescr['type'] == 'file') {
                ...
                if ((!isset($p_options[PCLZIP_OPT_TEMP_FILE_OFF])) && (isset($p_options[PCLZIP_OPT_TEMP_FILE_ON]) || (isset($p_options[PCLZIP_OPT_TEMP_FILE_THRESHOLD]) && ($p_options[PCLZIP_OPT_TEMP_FILE_THRESHOLD] <= $p_header['size'])))) {
                    ...
                    if ($p_options[PCLZIP_OPT_NO_COMPRESSION]) {
                        ...
                        // ----- Set header parameters
                        $p_header['compressed_size'] = mb_orig_strlen($v_content);
                ...
                if (@mb_orig_substr($p_header['stored_filename'], -1) != '/') {
    ...
    public function privCalculateStoredFilename(&$p_filedescr, &$p_options)
    {
        ...
        if (isset($p_filedescr['new_full_name'])) {
            ...
            if ($p_remove_all_dir) {
                ...
                if (mb_orig_substr($p_remove_dir, -1) != '/') {
                    $p_remove_dir .= "/";
                }

                if ((mb_orig_substr($p_filename, 0, 2) == "./") || (mb_orig_substr($p_remove_dir, 0, 2) == "./")) {
                    if ((mb_orig_substr($p_filename, 0, 2) == "./") && (mb_orig_substr($p_remove_dir, 0, 2) != "./")) {
                        $p_remove_dir = "./".$p_remove_dir;
                    }
                    if ((mb_orig_substr($p_filename, 0, 2) != "./") && (mb_orig_substr($p_remove_dir, 0, 2) == "./")) {
                        $p_remove_dir = mb_orig_substr($p_remove_dir, 2);
                    }
                }
                ...
                if ($v_compare > 0) {
                    if ($v_compare == 2) {
                        ...
                        $v_stored_filename = mb_orig_substr($v_stored_filename, mb_orig_strlen($p_remove_dir));
                    }
                }
            }
    ...
    public function privWriteFileHeader(&$p_header)
    {
        ...
        // ----- Packed data
        $v_binary_data = pack("VvvvvvVVVvv", 0x04034b50, $p_header['version_extracted'], $p_header['flag'], $p_header['compression'], $v_mtime, $v_mdate, $p_header['crc'], $p_header['compressed_size'], $p_header['size'], mb_orig_strlen($p_header['stored_filename']), $p_header['extra_len']);

        // ----- Write the first 148 bytes of the header in the archive
        fputs($this->zip_fd, $v_binary_data, 30);

        // ----- Write the variable fields
        if (mb_orig_strlen($p_header['stored_filename']) != 0) {
            fputs($this->zip_fd, $p_header['stored_filename'], mb_orig_strlen($p_header['stored_filename']));
        }
    ...
    public function privWriteCentralFileHeader(&$p_header)
    {
        ...
        // ----- Packed data
        $v_binary_data = pack("VvvvvvvVVVvvvvvVV", 0x02014b50, $p_header['version'], $p_header['version_extracted'], $p_header['flag'], $p_header['compression'], $v_mtime, $v_mdate, $p_header['crc'], $p_header['compressed_size'], $p_header['size'], mb_orig_strlen($p_header['stored_filename']), $p_header['extra_len'], $p_header['comment_len'], $p_header['disk'], $p_header['internal'], $p_header['external'], $p_header['offset']);
    ...
    public function privWriteCentralHeader($p_nb_entries, $p_size, $p_offset, $p_comment)
    {
        ...
        // ----- Packed data
        $v_binary_data = pack("VvvvvVVv", 0x06054b50, 0, 0, $p_nb_entries, $p_nb_entries, $p_size, $p_offset, mb_orig_strlen($p_comment));

        // ----- Write the 22 bytes of the header in the zip file
        fputs($this->zip_fd, $v_binary_data, 22);

        // ----- Write the variable fields
        if (mb_orig_strlen($p_comment) != 0) {
            fputs($this->zip_fd, $p_comment, mb_orig_strlen($p_comment));
        }
    ...
    public function privExtractByRule(&$p_file_list, $p_path, $p_remove_path, $p_remove_all_path, &$p_options)
    {
        ...
        // ----- Check the path
        if (($p_path == "") || ((mb_orig_substr($p_path, 0, 1) != "/") && (mb_orig_substr($p_path, 0, 3) != "../") && (mb_orig_substr($p_path, 1, 2)!=":/"))) {
            $p_path = "./".$p_path;
        }
        ...
        if (($p_path != "./") && ($p_path != "/")) {
            ...
            while (mb_orig_substr($p_path, -1) == "/") {
                $p_path = mb_orig_substr($p_path, 0, mb_orig_strlen($p_path)-1);
            }
        }

        // ----- Look for path to remove format (should end by /)
        if (($p_remove_path != "") && (mb_orig_substr($p_remove_path, -1) != '/')) {
            $p_remove_path .= '/';
        }
        $p_remove_path_size = mb_orig_strlen($p_remove_path);
        ...
        for ($i=0, $v_nb_extracted=0; $i<$v_central_dir['entries']; $i++) {
            ...
            if ((isset($p_options[PCLZIP_OPT_BY_NAME])) && ($p_options[PCLZIP_OPT_BY_NAME] != 0)) {
                ...
                for ($j=0; ($j<sizeof($p_options[PCLZIP_OPT_BY_NAME])) && (!$v_extract); $j++) {
                    // ----- Look for a directory
                    if (mb_orig_substr($p_options[PCLZIP_OPT_BY_NAME][$j], -1) == "/") {
                        // ----- Look if the directory is in the filename path
                        if ((mb_orig_strlen($v_header['stored_filename']) > mb_orig_strlen($p_options[PCLZIP_OPT_BY_NAME][$j])) && (mb_orig_substr($v_header['stored_filename'], 0, mb_orig_strlen($p_options[PCLZIP_OPT_BY_NAME][$j])) == $p_options[PCLZIP_OPT_BY_NAME][$j])) {
                            $v_extract = true;
                        }
    ...
    public function privExtractFile(&$p_entry, $p_path, $p_remove_path, $p_remove_all_path, &$p_options)
    {
        ...
        if ($p_remove_all_path == true) {
            ...
            $p_remove_path_size = mb_orig_strlen($p_remove_path);
            if (mb_orig_substr($p_entry['filename'], 0, $p_remove_path_size) == $p_remove_path) {
                // ----- Remove the path
                $p_entry['filename'] = mb_orig_substr($p_entry['filename'], $p_remove_path_size);
            }
        }
    ...
    //            eval('$v_result = '.$p_options[PCLZIP_CB_PRE_EXTRACT].'(PCLZIP_CB_PRE_EXTRACT, $v_local_header);');
        ...
        if ($p_entry['status'] == 'ok') {
            ...
            if (file_exists($p_entry['filename'])) {
                ...
                if ((($p_entry['external']&0x00000010)==0x00000010) || (mb_orig_substr($p_entry['filename'], -1) == '/')) {
                    $v_dir_to_check = $p_entry['filename'];
                } elseif (!mb_orig_strstr($p_entry['filename'], "/")) {
                    $v_dir_to_check = "";
                } else {
    ...
    public function privReadFileHeader(&$p_header)
    {
        ...
        if (mb_orig_strlen($v_binary_data) != 26) {
            $p_header['filename'] = "";
            $p_header['status'] = "invalid_header";

            // ----- Error log
            PclZip::privErrorLog(PCLZIP_ERR_BAD_FORMAT, "Invalid block size : ".mb_orig_strlen($v_binary_data));
    ...
    public function privReadCentralFileHeader(&$p_header)
    {
        ...
        // ----- Look for invalid block size
        if (mb_orig_strlen($v_binary_data) != 42) {
            $p_header['filename'] = "";
            $p_header['status'] = "invalid_header";
        ...
        if (mb_orig_substr($p_header['filename'], -1) == '/') {
    ...
    public function privReadEndCentralDir(&$p_central_dir)
    {
        ...
        // ----- Look for invalid block size
        if (mb_orig_strlen($v_binary_data) != 18) {
            // ----- Error log
            PclZip::privErrorLog(PCLZIP_ERR_BAD_FORMAT, "Invalid End of Central Dir Record size : ".mb_orig_strlen($v_binary_data));
        ...
        for ($i=0, $v_nb_extracted=0; $i<$v_central_dir['entries']; $i++) {
            ...
            if ((isset($p_options[PCLZIP_OPT_BY_NAME])) && ($p_options[PCLZIP_OPT_BY_NAME] != 0)) {
                ...
                for ($j=0; ($j<sizeof($p_options[PCLZIP_OPT_BY_NAME])) && (!$v_found); $j++) {
                    ...
                    if (mb_orig_substr($p_options[PCLZIP_OPT_BY_NAME][$j], -1) == "/") {
                        // ----- Look if the directory is in the filename path
                        if ((mb_orig_strlen($v_header_list[$v_nb_extracted]['stored_filename']) > mb_orig_strlen($p_options[PCLZIP_OPT_BY_NAME][$j])) && (mb_orig_substr($v_header_list[$v_nb_extracted]['stored_filename'], 0, mb_orig_strlen($p_options[PCLZIP_OPT_BY_NAME][$j])) == $p_options[PCLZIP_OPT_BY_NAME][$j])) {
                            $v_found = true;
    ...
    public function privDirCheck($p_dir, $p_is_dir = false)
    {
        ...
        // ----- Remove the final '/'
        if (($p_is_dir) && (mb_orig_substr($p_dir, -1)=='/')) {
            $p_dir = mb_orig_substr($p_dir, 0, mb_orig_strlen($p_dir)-1);
        }
...
function PclZipUtilPathInclusion($p_dir, $p_path)
{
    ...
    if (($p_dir == '.') || ((mb_orig_strlen($p_dir) >=2) && (mb_orig_substr($p_dir, 0, 2) == './'))) {
        $p_dir = PclZipUtilTranslateWinPath(getcwd(), false).'/'.mb_orig_substr($p_dir, 1);
    }
    if (($p_path == '.') || ((mb_orig_strlen($p_path) >=2) && (mb_orig_substr($p_path, 0, 2) == './'))) {
        $p_path = PclZipUtilTranslateWinPath(getcwd(), false).'/'.mb_orig_substr($p_path, 1);
    }
...
function PclZipUtilOptionText($p_option)
{
    ...
    for (reset($v_list); $v_key = key($v_list); next($v_list)) {
        $v_prefix = mb_orig_substr($v_key, 0, 10);
...
{
    if (mb_orig_stristr(php_uname(), 'windows')) {
        // ----- Look for potential disk letter
        if (($p_remove_disk_letter) && (($v_position = mb_orig_strpos($p_path, ':')) != false)) {
            $p_path = mb_orig_substr($p_path, $v_position+1);
        }
        // ----- Change potential windows directory separator
        if ((mb_orig_strpos($p_path, '\\') > 0) || (mb_orig_substr($p_path, 0, 1) == '\\')) {
            $p_path = strtr($p_path, '\\', '/');
        }
    }
    return $p_path;
