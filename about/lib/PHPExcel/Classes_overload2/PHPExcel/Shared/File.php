...
class PHPExcel_Shared_File
{
    ...
    public static function file_exists($pFilename)
    {
        ...
        if (mb_orig_strtolower(mb_orig_substr($pFilename, 0, 3)) == 'zip') {
            // Open ZIP file and verify if the file exists
            $zipFile     = mb_orig_substr($pFilename, 6, mb_orig_strpos($pFilename, '#') - 6);
            $archiveFile = mb_orig_substr($pFilename, mb_orig_strpos($pFilename, '#') + 1);
