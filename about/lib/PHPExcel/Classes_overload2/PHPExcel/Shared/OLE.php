...
class PHPExcel_Shared_OLE
{
    ...
    */
    public function _readPpsWks($blockId)
    {
        ...
        for ($pos = 0;; $pos += 128) {
            ...
            $nameUtf16 = mb_orig_substr($nameUtf16, 0, $nameLength - 2);
    ...
    */
    ...
    {
        $rawname = '';
        for ($i = 0; $i < mb_orig_strlen($ascii); ++$i) {
            $rawname .= $ascii{$i} . "\x00";
        }
        return $rawname;
    }
    ...
    */
    ...
    {
        if (mb_orig_strlen($string) != 8) {
            return new PEAR_Error("Expecting 8 byte string");
        }

        // factor used for separating numbers into 4 bytes parts
        $factor = pow(2, 32);
        list(, $high_part) = unpack('V', mb_orig_substr($string, 4, 4));
        list(, $low_part) = unpack('V', mb_orig_substr($string, 0, 4));
